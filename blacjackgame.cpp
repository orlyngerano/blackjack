...
public:
  ...
  void addCard(Cards::Card card) { cards.push_back(card); }

  void emptyCards() { cards.clear(); }

  std::vector<Cards::Card> getCards() { return cards; }
  ...
  bool isBetOnCards() { return betOnCards; }

  void setBetOnCards(bool betOnCards) { this->betOnCards = betOnCards; }
};
...
private:
  ...
  void showPlayerCardMessage()
  {
    ...
    for (auto const& card : cards) {
      message +=
        getCardName(card) + "=" + std::to_string((int)getCardValue(card)) + ",";
    }
